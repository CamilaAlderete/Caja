...
class Main{
       ...
       void cargar_lista(arista adyacentes[][],int INDEX,ArrayList<Vertice>vertices){
        ...
        void Dijkstra(arista adyacentes[][],ArrayList<Vertice>vertices){
          ...
          System.out.println(tiempo[1]); // distancia desde la posicion inicial(casa) al vertice que indica la facultad
       ...
       void imprimir_vertices(ArrayList<Vertice>vertices){
          ...
          while(i.hasNext()){
            AUX=i.next();
           System.out.println("nvertice:"+AUX.V+"linea:"+AUX.linea+"es casa:"+AUX.casa+"es facultad:"+AUX.F+"coordenadas"+"x:"+AUX.x+"y:"+AUX.y);
          }
        }    
    ...
    public static void main(String []args){
          ...
          //String aux; // bandera String que indica que se va a ingresar otro caso  de prueba
         // boolean pos_iniciales=false,x=false,primerindice=false; // bandera que indica si las posiciones de la casa y la facultad ya se ingresaron
          Scanner reader= new Scanner(System.in); 
          Vertice v;
          int indice=0,indiceanterior=0;  
          Main funcion= new Main();
    

         casos= reader.nextInt();      
         reader.nextLine(); //espacio despues de numero casos
         reader.nextLine();
         ...
         while(CONTCASO!=casos){   
            ...
            do{
              ...
              entradas= reader.nextLine();
              //System.out.println("\n"+entradas);
             // reader.nextLine();        
             
             while(indice>=0 && entradas.equals("")==false){
                
                if(x==false){
                  //si es la primera posicion se analiza desde la posicion 0
                  if(indice==0 || contV==0){
                  indice=entradas.indexOf(" ",indice);
                  primerindice=true;//bandera para saber si es el primer espacio que encuentra
                  }
                  else // si no es la primera posicion se analiza desde una posicion posterior al ultimo espacio encontrado
                    indice=entradas.indexOf(" ",indice+1);
                 // System.out.println(indice);
                 // System.out.println("guarda");
                  if(contV==0 || primerindice==true ){// primera posicion y el indiceanterior es 0
                  X=Integer.parseInt(entradas.substring(indiceanterior,indice));
                  primerindice=false;
                  }  
                  else// si no es la primera posicion, la nueva posicion anterior es una posicion siguiente al ultimo espacio encontrado
                    X=Integer.parseInt(entradas.substring(indiceanterior+1,indice));
                ...
                if(x==true){
                 // System.out.println("guarda Y");
                  indice=entradas.indexOf(" ",indiceanterior+1);
                 // System.out.println(indice);
                  if(indice<0){
                   Y=Integer.parseInt(""+entradas.substring(indiceanterior+1,entradas.length()));
                   x=false;
                  }
                  else{
                  x=false;
                  Y=Integer.parseInt(""+entradas.substring(indiceanterior+1,indice));
                  indiceanterior=indice;
                  }
                ...
                //  System.out.println("cambia de linea");
                 ...
                 // System.out.println("se crea vertice");
               ...
               indiceanterior=indice=0;
              
               
            }while(entradas.equals("")==false);   
